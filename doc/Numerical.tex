...
Note that the numerics annex of the Ada specification implements a number of numerical functions.  The basic elementary functions are covered, along with complex numbers, and vectors and matrices of real or complex numbers.  The 2012 Language Reference\cite{LRM2012} has more information.
...
\end{lstlisting}
\indexfunc{func-lngamma2n}
\begin{itemize}
  \item $n$ - A positive integer representing twice the value for computing $\Gamma$.
  \item Returns the natural log of the value.
\end{itemize}

Lanczos\cite{Lanczos} came up with an approximation to the $\Gamma$ function that works quite well.  The following routine computes $\Gamma(n)$ for any real $n > 0$.  It is based on a routine from \textit{Numerical Recipes in C}\cite{NR-C}.
\begin{lstlisting}
function lngamma(n : Positive) return f'Base;
\end{lstlisting}
\indexfunc{func-lngamma}
\begin{itemize}
  \item $n$ - A positive number for computing $\Gamma$.
  \item Returns the value of the $\Gamma$ function at $n$.
\end{itemize}

There are two incomplete gamma functions, $P(a,x)$ and $Q(a,x)$.  These are given by:
\begin{displaymath}
\begin{split}
...
where $\Gamma(a,x)$ is the upper incomplete gamma function and $\gamma(a,n)$ is the lower incomplete gamma function.  $\Gamma(a)$ is, of course, the gamma function.  The regularized functions, $P(a,x)$ and $Q(a,x)$ turn out to be useful in probability distributions.
\begin{lstlisting}
function gammaP(a, x : f'Base) return f'Base;
\end{lstlisting}
\indexfunc{func-gammaP}
Returns the value of the regularized gamma function $P(a,x)$
\begin{itemize}
  \item $a$ - The first parameter (must be $>0$).
  \item $x$ - The second parameter (must be $\ge 0$).
  \item Returns the natural log of the value $P(a,x)$.
\end{itemize}

\begin{lstlisting}
function gammaQ(a, x : f'Base) return f'Base;
\end{lstlisting}
\indexfunc{func-gammaQ}
Returns the value of the regularized gamma function $Q(a,x)$
\begin{itemize}
  \item $a$ - The first parameter (must be $>0$).
  \item $x$ - The second parameter (must be $\ge 0$).
  \item Returns the natural log of the value $(a,x)$.
\end{itemize}

The error functions are based on integrals of the normal curve.  The error function gives the probability that a result, $Y$ falls in the range $[-x,x]$.  The complementary error function gives the probability that $Y$ is outside of the range.  They can be computed using the above regularized incomplete gamma functions.
\begin{lstlisting}
...
\begin{itemize}
  \item $x$ - The parameter.
  \item Returns the error function for $x$.
\end{itemize}
...
\begin{itemize}
  \item $x$ - The parameter.
  \item Returns the complementary error function for $x$.
\end{itemize}
...
There is a relation between the Beta function and the Gamma function that makes it fairly easy to compute:
\begin{displaymath}
  \beta(A, B) = \frac{\Gamma(A)\Gamma(B)}{\Gamma(A+B)}
\end{displaymath}
From this, it is also evident that $\beta(A, B)$ is equal to $\beta(A, b)$.

\begin{lstlisting}
function beta(a, b : f'Base) return f'Base;
\end{lstlisting}
\indexfunc{func-beta}
Computes the beta function of $a$ and $b$.  Requires $a>0$ and $b>0$.
\begin{itemize}
  \item $a$ - The first parameter to the beta function.
  \item $b$ - The second parameter to the beta function.
  \item Returns the value of the beta function of $a$ and $b$.
\end{itemize}

\begin{lstlisting}
function lnbeta(a, b : f'Base) return f'Base;
\end{lstlisting}
\indexfunc{func-lnbeta}
Computes the natural log of the beta function of $a$ and $b$.  Requires $a>0$ and $b>0$.
\begin{itemize}
  \item $a$ - The first parameter to the beta function.
  \item $b$ - The second parameter to the beta function.
  \item Returns the natural log of the beta function of $a$ and $b$.
\end{itemize}
...
This estimates the value of the integral using the composite midpoint method.  In general, this isn't a very good method.  However if your function to integrate has a singularity at the upper or lower bound, it might be useful since it doesn't evaluate the function there.
\begin{lstlisting}
function midpoint(test : test_func; lower, upper : f'Base; steps : Positive)
         return f'Base;
\end{lstlisting}
\indexfunc{integ-midpoint}
\begin{itemize}
...
\begin{lstlisting}
function trapezoid(test : test_func; lower, upper : f'Base;
         steps : Positive) return f'Base;
\end{lstlisting}
\indexfunc{integ-trapezoid}
\begin{itemize}
  \item $test$ - The function to integrate.  It must match the datatype for \datatype{test\_func}.
  \item $lower$ - The lower bounds of the integration.
  \item $upper$ - The upper bounds of the integration.
...
\subsection{Second order Lagrange}
\begin{lstlisting}
function lag2(p0, p1 : point; x : f'Base) return f'Base;
\end{lstlisting}
\indexfunc{interp-lag2}
\begin{itemize}
...
\subsection{Third order Lagrange}
\begin{lstlisting}
function lag3(p0, p1, p2 : point; x : f'Base) return f'Base;
\end{lstlisting}
\indexfunc{interp-lag3}
\begin{itemize}
...
The following draw glyphs on the plot.
\begin{lstlisting}
procedure draw_glyph(self : in out plot_record; p : point; g : glyph;
                     color : String);
\end{lstlisting}
\indexfunc{plot-draw\_glyph}
...
\end{lstlisting}
\indexfunc{plot-draw\_glyph}
Draw a set of glyphs at specified points on the plot.
\begin{itemize}
  \item $self$ - The plot being updated.
...
The following draw glyphs with error bars on the plot.
\begin{lstlisting}
procedure draw_glyph(self : in out plot_record; p : point_err;
                     g : glyph; color : String);
\end{lstlisting}
\indexfunc{plot-draw\_glyph}
Draw a single glyph with error bars at a specified point on the plot.
\begin{itemize}
  \item $self$ - The plot being updated.
  \item $p$ - The location of the glyph.
  \item $g$ - The glyph to draw.
...
\end{lstlisting}
\indexfunc{plot-draw\_glyph}
Draw a set of glyphs with error bars at specified points on the plot.
\begin{itemize}
  \item $self$ - The plot being updated.
  \item $points$ - An array of points for the glyph locations.
  \item $g$ - The glyph to draw.
...
%--------------------------------------------------------------------------------------------------
\section{BBS.Numerical.plot\_latex\_linear}
This package contains specific routines for drawing a plot with linear axis using the \package{tikz} \LaTeX package.  The plot can be included in a \LaTeX document.  See below for examples.  In addition to the routines defined in \package{BBS.Numerical.plot}, it defines two additional routines.  For using the common interface, use a \datatype{BBS.Numerical.plot\_latex\_linear.linear\_latex\_plot\_record} instead of the basic \datatype{BBS.Numerical.plot.plot\_record}.

\begin{lstlisting}
procedure start_plot(self : in out linear_latex_plot_record; name : string;
                     xmin, xmax, ymin, ymax : float);
\end{lstlisting}
\indexfunc{plot-latex-start\_plot}
Starts a plot.  It opens the output file for the \LaTeX plot and sets the plot's bounds.
\begin{itemize}
...
\end{lstlisting}
\indexfunc{plot-latex-end\_plot}
...
%--------------------------------------------------------------------------------------------------
\section{BBS.Numerical.plot\_svg\_linear}
This package contains specific routines for drawing a plot with linear axis to a SVG format file.  In addition to the routines defined in \package{BBS.Numerical.plot}, it defines two additional routines.  For using the common interface, use a \datatype{BBS.Numerical.plot\_svg\_linear.linear\_svg\_plot\_record} instead of the basic \datatype{BBS.Numerical.pllot.plot\_record}.
\begin{lstlisting}
procedure start_plot(self : in out linear_svg_plot_record; name : string;
                     xmin, xmax, ymin, ymax : float);
\end{lstlisting}
\indexfunc{plot-svg-start\_plot}
